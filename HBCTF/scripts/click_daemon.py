...
def start():
	...
	if not pid:
		click.echo("Unable run daemon")
	else:
		click.echo("Daemon is running [PID=%d]" % pid)

@click.argument('stop')
def stop():
    click.echo("Stoping daemon")
    daemon.stop()

@click.argument('restart')
def restart():
...
def status():
    click.echo("Viewing daemon status")
    pid = daemon.get_pid()

    if not pid:
        click.echo("Daemon isn't running ;)")
    else:
        click.echo("Daemon is running [PID=%d]" % pid)
